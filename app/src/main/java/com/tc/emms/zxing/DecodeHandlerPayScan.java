...
final class DecodeHandlerPayScan extends Handler {

  private static final String TAG = DecodeHandlerPayScan.class.getSimpleName();

  private final PayScanActivity activity;
  private final MultiFormatReader multiFormatReader;

  DecodeHandlerPayScan(PayScanActivity activity, Hashtable<DecodeHintType, Object> hints) {
  ...
  public void handleMessage(Message message) {
    switch (message.what) {
      case R.id.decode:
        //LogUtils.dLog(TAG, "Got decode message");
...
//     PlanarYUVLuminanceSource source = CameraManager.get().buildLuminanceSource(data, height, width);
    ...
    if (rawResult != null) {
      ...
      LogUtils.dLog(TAG, "Found barcode (" + (end - start) + " ms):\n" + rawResult.toString());
      Message message = Message.obtain(activity.getHandler(), R.id.decode_succeeded, rawResult);
      Bundle bundle = new Bundle();
      bundle.putParcelable(DecodeThread.BARCODE_BITMAP, source.renderCroppedGreyscaleBitmap());
      message.setData(bundle);
      //LogUtils.dLog(TAG, "Sending decode succeeded message...");
