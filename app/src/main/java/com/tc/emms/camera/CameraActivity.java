...
import com.tc.emms.crop.CropView;
import com.tc.emms.crop.FrameOverlayView;
import com.tc.emms.crop.IDcardQualityProcess;
...
public class CameraActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.album_button).setOnClickListener(
				albumButtonOnClickListener);
		...
		findViewById(R.id.rotate_button).setOnClickListener(
				rotateButtonOnClickListener);
		...
		cropMaskView = (MaskView) cropContainer
				.findViewById(R.id.crop_mask_view);
		cropContainer.findViewById(R.id.cancel_button).setOnClickListener(
				cropCancelButtonListener);
	...
	private void initParams() {
		...
		boolean isNativeEnable = getIntent().getBooleanExtra(KEY_NATIVE_ENABLE,
				true);
		if (token == null) {
		...
		CameraThreadPool.execute(new Runnable() {
			...
			public void run() {
				...
				if (IDcardQualityProcess.getLoadSoException() != null) {
					cameraView
							.setInitNativeStatus(CameraView.NATIVE_SOLOAD_FAIL);
					return;
				}
				// 授权状态
	...
	private View.OnClickListener lightButtonOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			if (cameraView.getCameraControl().getFlashMode() == ICameraControl.FLASH_MODE_OFF) {
				cameraView.getCameraControl().setFlashMode(
						ICameraControl.FLASH_MODE_TORCH);
			} else {
				cameraView.getCameraControl().setFlashMode(
						ICameraControl.FLASH_MODE_OFF);
			}
			updateFlashMode();
		}
	};
	...
	private CameraView.OnTakePictureCallback takePictureCallback = new CameraView.OnTakePictureCallback() {
		...
		public void onPictureTaken(final Bitmap bitmap) {
			handler.post(new Runnable() {
				...
				public void run() {
					if(bitmap != null){
						ConstantUtils.saveBitmap = bitmap;
						/*保存bitmap*/
						String save_path = FileUtils.getStoragePath(BaseActivity.mActivity, false)
								+ "/" + ConstantUtils.UPLOAD_PATH;
						LogUtils.vLog(TAG, "saveimageTosandboxWithimage save_path:" + save_path);
						String name = DateUtils.getNowTime() + ".jpg";
						File file = new File(save_path);
						if (!file.exists()) {
							file.mkdirs();
						}
						ConstantUtils.savePath = save_path + name;
						LogUtils.vLog(TAG, "saveimageTosandboxWithimage--path: " + ConstantUtils.savePath);
						boolean isOK = ImageUtils.saveBitmapToSDCard(bitmap, ConstantUtils.savePath);
						if(isOK){
							
						}
						LogUtils.dLog(TAG, "IDCardActivity 拍照方式  takePictureCallback save bitmap:" + isOK);
		...
		CameraThreadPool.execute(new Runnable() {
			...
			public void run() {
				try {
					FileOutputStream fileOutputStream = new FileOutputStream(
							outputFile);
					Bitmap bitmap = ((BitmapDrawable) displayImageView
