...
public final class CameraManager {
	...
	private static boolean isLoginPrint = false;
	public static void init(BaseActivity context, Handler ui_handler) {
		if (cameraManager == null) {
			...
			cameraManager = new CameraManager(context, ui_handler);
		}
		String save_camera = SharedUtils.getValue(context,
				PrintUtil.KEY_IS_CAMERA);
	...
	private CameraManager(BaseActivity context, Handler ui_handler) {

		this.context = context;
		this.configManager = new CameraConfigurationManager(context);
		this.update_ui_handler = ui_handler;
	...
	private void showACameraDialog() {
		...
		TextView titleTV = (TextView) dialog.findViewById(R.id.tv_tip_title);
		titleTV.setText(context.getString(R.string.app_name));
		TextView contentTV = (TextView) dialog
				.findViewById(R.id.tv_tip);
		contentTV.setText(context.getString(R.string.msg_camera_framework_bug));
	...
	public void requestAutoFocus(Handler handler, int message) {
		if (camera != null && previewing) {
			...
			// LogUtils.dLog(TAG, "Requesting auto-focus callback");
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			framingRect = new Rect(leftOffset, topOffset, leftOffset + width,
					topOffset + height);
			LogUtils.dLog(TAG, "Calculated framing rect: " + framingRect);
			if(update_ui_handler != null){

				Message message = new Message();
				message.arg1 = width;
				message.arg2 = height;
				update_ui_handler.sendMessage(message);
			}
		}
		return framingRect;
	}
