...
final class CameraConfigurationManager {
	...
	void initFromCameraParameters(Camera camera) {
		...
		LogUtils.dLog(TAG, "Default preview format: " + previewFormat + '/'
				+ previewFormatString);
		WindowManager manager = (WindowManager) context
				.getSystemService(Context.WINDOW_SERVICE);
		Display display = manager.getDefaultDisplay();
		screenResolution = new Point(display.getWidth(), display.getHeight());
		LogUtils.dLog(TAG, "Screen resolution: " + screenResolution);
		...
		LogUtils.dLog(TAG, "Camera resolution: " + screenResolution);
	}
	...
	void setDesiredCameraParameters(Camera camera) {
		Camera.Parameters parameters = camera.getParameters();
		LogUtils.dLog(TAG, "Setting preview size: " + cameraResolution);
	...
	private static Point getCameraResolution(Camera.Parameters parameters,
		...
		if (previewSizeValueString != null) {
			LogUtils.dLog(TAG, "preview-size-values parameter: "
	...
	private static Point findBestPreviewSizeValue(
		...
		for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
			...
			if (dimPosition < 0) {
				LogUtils.iLog(TAG, "Bad preview-size: " + previewSize);
				continue;
			}
	...
	private void setZoom(Camera.Parameters parameters) {
		...
		if (maxZoomString != null) {
			try {
				...
				LogUtils.iLog(TAG, "Bad max-zoom: " + maxZoomString);
			}
		}
		...
		if (takingPictureZoomMaxString != null) {
			try {
				...
				LogUtils.iLog(TAG, "Bad taking-picture-zoom-max: "
	 ...
	 * (NumberFormatException nfe) { LogUtils.iLog(TAG, "Bad sharpness-max: " +
	 * maxSharpnessString); } }
	 * 
