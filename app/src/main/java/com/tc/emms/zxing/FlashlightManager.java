...
final class FlashlightManager {
  ...
  static {
    ...
    if (iHardwareService == null) {
      LogUtils.vLog(TAG, "This device does supports control of a flashlight");
    } else {
      LogUtils.vLog(TAG, "This device does not support control of a flashlight");
    }
  }
  ...
  private static Class<?> maybeForName(String name) {
    try {
      ...
      LogUtils.iLog(TAG, "Unexpected error while finding class " + name + ",and:" + re);
      return null;
    }
  }
  ...
  private static Method maybeGetMethod(Class<?> clazz, String name, Class<?>... argClasses) {
    try {
      ...
      LogUtils.iLog(TAG, "Unexpected error while finding method " + name + ",and:" + re);
      return null;
    }
  }
  ...
  private static Object invoke(Method method, Object instance, Object... args) {
    try {
      ...
      LogUtils.iLog(TAG, "Unexpected error while invoking " + method + ",and:" + e);
      return null;
    } catch (InvocationTargetException e) {
      LogUtils.iLog(TAG, "Unexpected error while invoking " + method + ",and:" + e.getCause());
      return null;
    } catch (RuntimeException re) {
      LogUtils.iLog(TAG, "Unexpected error while invoking " + method + ",and:" + re);
      return null;
    }
  }
